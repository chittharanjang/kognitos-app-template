...
/**
 * UAT for the DB Agent automation.
 *
 * Questions are read exclusively from `lib/uat-questions.ts` (tagged
 * source:"uat") and are NEVER mixed with the db_agent_test_questions table
 * that powers the "Test" button on the flat run-history page.
 *
 * Body (all optional):
 ...
 *   result  { question, runId?, status, responseText?, recordCount?,
...
const REQUESTER_EMAIL = "uat@kognitos-demo.local";
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      if (questions.length === 0) {
        emit("done", { total: 0, completed: 0, failed: 0, durationMs: 0 });
        controller.close();
        return;
      }
      ...
      async function worker(): Promise<void> {
        while (true) {
          ...
          emit("result", {
            ...
            recordCount:
              typeof r.outputs?.record_count === "number"
                ? r.outputs.record_count
                : null,
            error: r.error ?? null,
            durationMs: Date.now() - t0,
          });
