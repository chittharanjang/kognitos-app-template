...
function isMissingTable(code: string | undefined): boolean {
  // 42P01 = undefined_table, 42703 = undefined_column, PGRST205 = PostgREST schema cache miss.
...
export async function PUT(request: Request) {
  ...
  // Back-compat: explicit null still deletes the row entirely. New clients
  // should always send 'correct' or 'incorrect' and rely on bootstrap to
  // ensure rows exist.
  if (verdictRaw === null) {
  ...
  // Default verdict to 'correct' when not provided (e.g. notes-only edits).
  ...
  // Build the upsert payload. Omit `notes` if the caller didn't send it so
  // we don't accidentally overwrite an existing note with NULL.
