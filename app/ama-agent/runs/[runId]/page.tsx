...
import {
  ...
  NotesField,
  type Verdict,
} from "../../../components/verdict-controls";
import { StageVersionBadge } from "../../../components/stage-version-badge";
...
function RunDetailView({ data }: { data: RunDetail }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    // run's detail page also yields a default 'correct' row immediately. The
    // same call upserts the run into the run-groups index, so opening any
    // run detail keeps that view fresh too.
    fetch("/api/ama-agent/runs/verdicts/bootstrap", {
      method: "POST",
        ...
        runs: [
          {
            ...
            status: data.status,
            recordCount:
              typeof data.recordCount === "number" ? data.recordCount : null,
            databasesQueried: Array.isArray(data.databasesQueried)
              ? data.databasesQueried.join(", ")
              : (data.databasesQueried ?? null),
            stage: data.stage,
      ...
      .then(() => fetch("/api/ama-agent/runs/verdicts", { cache: "no-store" }))
      .then(async (res) => {
        if (!res) return;
