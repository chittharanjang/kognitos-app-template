 ...
 * in `theirs` are tagged 'added'. Set-difference is preferred to a positional
 * diff because answer/SQL output can re-order lines harmlessly.
 */
...
export default function QueryComparePage({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch("/api/query/run-groups/compare", {
      method: "POST",
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        <Link
          href={`/query/run-groups/${questionId}`}
      ...
      <div>
        ...
        <Text level="xSmall" color="muted" className="mt-1">
          Differences are highlighted per column. Lines unique to a column are
...
function CompareColumn({
  ...
  const myFilters = (run.appliedWhereClauses ?? []).join("\n");
  const otherFilters = others.map((o) =>
    (o.appliedWhereClauses ?? []).join("\n"),
  );
    ...
    <div className="rounded-xl border border-border bg-card shadow-sm flex flex-col min-h-0">
      <div className="p-4 border-b border-border space-y-2">
        ...
        <div className="flex items-center gap-2 text-xs flex-wrap">
          ...
          <span className="text-muted-foreground ml-2">filters</span>
          <span className="font-semibold tabular-nums">
            {run.appliedWhereClauses?.length ?? 0}
          </span>
        </div>
        <div>
          <Link
