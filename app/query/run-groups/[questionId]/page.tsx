...
export default function QueryRunGroupDetailPage({
  params,
}: {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/query/run-groups/${questionId}`, {
        cache: "no-store",
      });
  ...
  const handleCompare = (): void => {
    ...
    router.push(`/query/run-groups/${questionId}/compare?${qs.toString()}`);
  };
  ...
  const handleReRun = useCallback(async () => {
    ...
    setReRunError(null);
    try {
      // The /query page POSTs questions to /api/query and expects the
      // returned runId for navigation, mirroring the chat flow.
      ...
      router.push(`/query/runs/${json.runId}`);
    } catch (e) {
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right">Filters</th>
                    <th
